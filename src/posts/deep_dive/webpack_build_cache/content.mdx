...
![webpack.config.js 파일 내 hash 로직](/posts/deep_dive/webpack_build_cache/1.webp)
 
정말 변경이 있는 파일만 해시값이 바뀔까?
파일 확장자에 따라서 그 전략이 다를 것 같은데,
실제 파일 변경 후 빌드를 해서 전후 차이를 살펴보도록 하자.

![파일 변경 전후 빌드파일 해시값 비교표. 초록 음영은 css 파일](/posts/deep_dive/webpack_build_cache/2.webp)
...
![변경되지 않은 파일 (위 도표의 main.js)](/posts/deep_dive/webpack_build_cache/3.webp)

![변경된 파일 (위 도표의 59번 js 파일)](/posts/deep_dive/webpack_build_cache/4.webp)
